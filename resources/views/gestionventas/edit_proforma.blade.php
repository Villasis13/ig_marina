...
@section('content')
    ...
    <div class="tab-content">
        <div id="vista_para_opciones_{{$opciones[0]->id_opciones}}" class="tab-pane fade show active " role="tabpanel" aria-labelledby="opciones_{{$opciones[0]->id_opciones}}" tabindex="0">
            <form id="formulario_registro_proformas" class="mb-3"  method="POST" enctype = "multipart/form-data">
                ...
                <input type="hidden" name="id_profo" id="id_profo" value="{{$proforma ? $proforma->id_profo : '' }}" >
                <div class="row">
                                        ...
                                        <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
                                            ...
                                            <select name="id_tipo_documento" id="id_tipo_documento" class="form-control">
                                               @php
                                                    ...
                                                    $select2 = "";
                                                    if ($proforma){
                                                        $select =  $proforma->id_tipo_documento == 4 ? 'selected' : '';
                                                        $select2 =  $proforma->id_tipo_documento == 2 ? 'selected' : '';
                                                    }
                                                @endphp
                                                <option value="4" {{$select}}>RUC</option>
                                                <option value="2" {{$select2}}>DNI</option>
                                            </select>
                                        </div>
                                        <div class="col-lg-4 col-md-6 col-sm-12 mb-3">
                                            <label for="num_documento">N° Documento (*)</label>
                                            <input type="text" class="form-control" value="{{$proforma ? $proforma->cliente_numero : '' }}" name="num_documento" id="num_documento" onkeyup="validar_numeros(this.id)">
                                        </div>
                                        <div class="col-lg-4 col-md-12 col-sm-12 mb-3">
                                            <label for="tel_cliente">Teléfono</label>
                                            <input type="text" class="form-control" name="tel_cliente" id="tel_cliente"  value="{{$proforma ? $proforma->cliente_telefono : '' }}" onkeyup="validar_numeros(this.id)">
                                        </div>
                                        <div class="col-lg-12 col-md-12 col-sm-12 mb-3">
                                            <label for="razon_social_cliente">Razón Social (*) </label>
                                            <input type="text" class="form-control" name="razon_social_cliente" value="{{$proforma ? $proforma->cliente_razonsocial : '' }}" id="razon_social_cliente" >
                                        </div>

                                        <div class="col-lg-12 col-md-12 col-sm-12 mb-4">
                                            <label for="direccion_cliente">Dirección </label>
                                            <textarea name="direccion_cliente" id="direccion_cliente" rows="2" class="form-control">{{$proforma ? $proforma->cliente_direccion : '' }}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-md-6 col-sm-12">
                                    <div class="row">
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12 mb-3">
                                            ...
                                            @php
                                                ...
                                                $select2forma = "";
                                                if ($proforma){
                                                    $selectforma =  $proforma->profo_forma_pago == 1 ? 'selected' : '';
                                                    $select2forma =  $proforma->profo_forma_pago == 1 ? 'selected' : '';
                                                }
                                            @endphp
                                            <select name="forma_pago" id="forma_pago" class="form-control">
                                                <option value="1" {{$selectforma}}>CONTADO</option>
                                                <option value="2" {{$select2forma}}>CREDITO</option>
                                            </select>
                                        </div>
                                        <div class="col-lg-12 col-md-12 col-sm-12 mb-3">
                                            <label for="lugar_entrega">Lugar de entrega (*) </label>
                                            <input type="text" class="form-control" value="{{$proforma ? $proforma->profo_lugar_entrega : 'Previa Coordinación' }}" name="lugar_entrega" id="lugar_entrega" >
                                        </div>
                                        <div class="col-lg-12 col-md-12 col-sm-12 mb-2">
                                            ...
                                            <textarea name="observaciones_proforma" id="observaciones_proforma"  rows="2" class="form-control">
                                                {{$proforma ?  $proforma->profo_observacion  : 'Los precios están sujetos a variaciones'}}
                                            </textarea>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-lg-12 col-md-12 col-sm-12 mt-5 text-end">
                                    ...
                                    <a href="{{route('Gestionventas.proformas')}}" class="btn btn-secondary m-1 " id="btn_guardar_proformas"><i class="fa-solid fa-arrow-rotate-left"></i> Regresar</a>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        @if($id)
            let datosProforma = <?= $detalle ?>;
            if(datosProforma.length > 0){
                if(localStorage.getItem('productos_proforma')){

                }else{
                    proformas_productos = [];
                    datosProforma.map(function (el,index){
                        let obj = {
                            id_pro : el.id_pro,
                            producto_nombre: el.pro_nombre,
                            producto_precio_unit : el.pro_precio_uni,
                            producto_precio_mayo : el.pro_precio_uni_ma,
                            producto_precio_final: el.pro_precio_uni,
                            comentarios: el.profo_deta_observacion,
                            stock_actual : el.pro_stock,
                            producto_codigo :el.pro_codigo,
                            cantidad : el.profo_deta_cantidad,
                            precio_subtotal : el.pro_precio_uni * el.profo_deta_cantidad,
                        }
        ...
        @else
            if(localStorage.getItem('productos_proforma')) {
                ...
                dibujar_tabla_productos_proformas(); // Esto asegura que se dibuje la tabla con los datos recuperados
            }else{
                proformas_productos = [];
                localStorage.setItem('productos_proforma', JSON.stringify(proformas_productos));
            }
